#!/usr/bin/env python3
"""
KOSEI HTML Generator v1.0
Reads workflow_content.json + workflow_control.json and produces the HTML diagram.
Double-click or run: python kosei_generate.py
"""
import json, re, os, sys, html as htmlmod
from datetime import datetime

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
CONTENT_FILE = os.path.join(SCRIPT_DIR, "workflow_content_v2.json")
...
<title>{esc(content.get("header",{}).get("title","KOSEI Workflow"))}</title>
<style>
...
'''
    ...
    out += f'  <div class="nav-logo">{esc(header.get("logo","KOSEI"))}</div>\n'
...
document.querySelectorAll('.topnav a').forEach(function(link){{link.addEventListener('click',function(){{var hash=link.getAttribute('href');if(!hash||!hash.startsWith('#'))return;var t=document.querySelector(hash);if(t)t.classList.add('open')}});}});
    ...
    # ─── WRITE ───
    output_name = control.get('output_file', 'KOSEI_Workflow_Diagram_v1.html')
